...
pathAQ = "../Datasets/Juriquilla/AirQuality/"
...
for i, title in enumerate(titlesAQ):
    if i == 0: 
        ...
        following = pd.read_csv(pathAQ + title, skiprows=14)
        following.drop(0, axis=0, inplace=True) 
        for c in following.columns: following.drop(c, axis=1, inplace=True)
...
pathMet = "../Datasets/Juriquilla/Meteorology/"
...
dfMet.drop(["Visibility", "Visibility_Avg"], axis=1, inplace=True)
